(function () {
  ...
  // Scroll shade: living color shift – opacity and gradient position follow scroll
  function updateScrollShade() {
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
    var scrollY = window.scrollY;
    var docHeight = document.documentElement.scrollHeight - window.innerHeight;
    var maxOpacityScroll = 500;
    var progress = docHeight > 0 ? Math.min(1, scrollY / docHeight) : 0;
    var opacity = Math.min(1, scrollY / maxOpacityScroll);
    document.documentElement.style.setProperty("--scroll-progress", progress);
    document.documentElement.style.setProperty("--scroll-shade-opacity", opacity);
  }
  ...
  document.querySelectorAll('a[href^="#"]').forEach(function (anchor) {
    anchor.addEventListener("click", function (e) {
      ...
      if (target) {
        ...
        if (href === "#main-content") target.setAttribute("tabindex", "-1");
      }
    });
  });
